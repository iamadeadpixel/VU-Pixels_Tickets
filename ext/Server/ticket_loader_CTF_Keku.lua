...
Events:Subscribe('Level:LoadResources', function(p_LevelName_Keku_CTF, p_GameMode_Keku_CTF, p_IsDedicatedServer)
-- Contains ALL GameModemap data
	local s_GameMode_Keku_CTF = ServerUtils and ServerUtils:GetCustomGameModeName() or global_GameModeNameMap[p_GameMode_Keku_CTF] or p_GameMode_Keku_CTF

-- Contains Keku_CTF map data
	s_LevelName_Keku_CTF = ServerUtils and ServerUtils:GetCustomMapName() or Keku_CTF_LevelNameMap[p_LevelName_Keku_CTF] or p_LevelName_Keku_CTF and s_GameMode_Keku_CTF == "CTF"

-- ------------------------------------------------------------------------------

	if (Keku_CTF_LevelNameMap[p_LevelName_Keku_CTF] ~= nill) then
	s_vehicles = "true"
...
-- Experimental: Setting vehicles on or off on specific map, any other map should be enabled
	if d_LevelName == s_LevelName_Keku_CTF and global_GameModeNameMap[p_GameMode_Keku_CTF] == "CTF" then
		s_vehicles = "false"
...
-- ------------------------------------------------------------------------------
	...
	if lm == p_LevelName_Keku_CTF  and s_GameMode_Keku_CTF == "CTF" then

	local s_MaxPlayersRCON = RCON:SendCommand('vars.maxPlayers')
	local s_MaxPlayers = tonumber(s_MaxPlayersRCON[2])
	
	print(''..s_MapData..': - Reading map data: '..s_LevelName_Keku_CTF..' - '..s_GameMode_Keku_CTF..' ')
