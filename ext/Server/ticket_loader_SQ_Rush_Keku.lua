...
local Keku_Rush_SQ_LevelNameMap = {
...
Events:Subscribe('Level:LoadResources', function(p_LevelName_Keku_Rush_SQ, p_GameMode_Keku_Rush_SQ, p_IsDedicatedServer)
-- Contains ALL GameModemap data
	local s_GameMode_Keku_Rush_SQ = ServerUtils and ServerUtils:GetCustomGameModeName() or global_GameModeNameMap[p_GameMode_Keku_Rush_SQ] or p_GameMode_Rush_SQ

-- Contains Keku_Rush_SQ map data
	local s_LevelName_Keku_Rush_SQ = ServerUtils and ServerUtils:GetCustomMapName() or Keku_Rush_SQ_LevelNameMap[p_LevelName_Keku_Rush_SQ] or p_LevelName_Keku_Rush_SQ and s_GameMode_Keku_Rush_SQ == "Squad Rush"

-- ------------------------------------------------------------------------------

	if (Keku_Rush_SQ_LevelNameMap[p_LevelName_Keku_Rush_SQ] ~= nill) then
	s_vehicles = "true"
...
-- Experimental: Setting vehicles on or off on specific map, any other map should be enabled
	if d_LevelName == s_LevelName_Keku_Rush_SQ  and global_GameModeNameMap[p_GameMode_Keku_Rush_SQ] == "Squad Rush" then
		s_vehicles = "false"
...
-- ------------------------------------------------------------------------------
	...
	if lm == p_LevelName_Keku_Rush_SQ  and s_GameMode_Keku_Rush_SQ == "Squad Rush" then

	local s_MaxPlayersRCON = RCON:SendCommand('vars.maxPlayers')
	local s_MaxPlayers = tonumber(s_MaxPlayersRCON[2])
	
	print(''..s_MapData..': - Reading map data: '..s_LevelName_Keku_Rush_SQ..' - '..s_GameMode_Keku_Rush_SQ..' ')
