...
-- Conquest Small Maps Vehicles disabled: Make it more Infantry friendly.
...
-- Contains ALL GameModemap data for Conquest Small.
	local s_GameMode_CQS = ServerUtils and ServerUtils:GetCustomGameModeName() or GameMode_CQS[p_GameMode_CQS] or p_GameMode_CQS

	local s_LevelName_CQS = ServerUtils and ServerUtils:GetCustomMapName() or CQS150[p_LevelName_CQS] or CQS200[p_LevelName_CQS] or CQS250[p_LevelName_CQS] or CQS300[p_LevelName_CQS] or CQS350[p_LevelName_CQS] or p_LevelName_CQS

-- CQS Maps Vehicles disabled
	local d_LevelName_CQS = ServerUtils and ServerUtils:GetCustomMapName() or CQS_Disabled[p_LevelName_CQS]

-- ------------------------------------------------------------------------------

 	if (CQS350[p_LevelName_CQS] ~= nill) then
	s_Tickets = tickets_CQS350
	s_ratio = 3.5
	goto set_tickets

 	elseif (CQS300[p_LevelName_CQS] ~= nill) then
	s_Tickets = tickets_CQS300
	s_ratio = 3
	goto set_tickets

 	elseif (CQS250[p_LevelName_CQS] ~= nill) then
	s_Tickets = tickets_CQS250
	s_ratio = 2.5
	goto set_tickets

 	elseif (CQS200[p_LevelName_CQS] ~= nill) then
	s_Tickets = tickets_CQS200
	s_ratio = 2
	goto set_tickets

 	elseif (CQS150[p_LevelName_CQS] ~= nill) then
	s_Tickets = tickets_CQS150
	s_ratio = 1.5
	goto set_tickets
	end
...
-- ------------------------------------------------------------------------------
	...
	s_MapData=s_GameMode_CQS

-- Data for the chat message when the map has Vehicles enabled (Default)
	m_GameMode = s_GameMode_CQS
	m_LevelName = s_LevelName_CQS
...
-- If a map is in the CQS_Disabled table, This turns off the vehicle flag.
	if d_LevelName_CQS == s_LevelName_CQS  and GameMode_CQS[p_GameMode_CQS] == "Conquest Small" then
	s_vehicles = "false"
...
::rcon:: 
	...
	print(''..s_MapData..': - level data: Mapname:'..s_LevelName_CQS..' - GameMode:'..s_GameMode_CQS..' ')
